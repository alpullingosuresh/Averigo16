...
class PayrollDeductionReport(models.TransientModel):
    _name = 'global.payroll.deduction.report'
    _description = "Global Payroll Deduction Report"
    _rec_name = "name"
    ...
    mm_ids = fields.Many2many('stock.warehouse', 'mm_ids_rel',
                              string='Micro Markets',
                              domain="[('id', 'in', micro_market_ids)]")
    micro_market_ids = fields.Many2many('stock.warehouse', 'micro_market_rel')
    line_ids = fields.One2many('global.payroll.deduction.report.lines',
                               'report_id', string="Invoice")
...
class PayrollDeductionReportLines(models.TransientModel):
    _name = 'global.payroll.deduction.report.lines'
    _description = "Global Payroll Deduction Report Lines"
    ...
    report_id = fields.Many2one('global.payroll.deduction.report')
    company_id = fields.Many2one('res.company', required=True,
                                 default=lambda self: self.env.company)
    currency_id = fields.Many2one('res.currency',
                                  related='company_id.currency_id')
