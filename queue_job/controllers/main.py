...
class RunJobController(http.Controller):
    def _try_perform_job(self, env, job):
        print('nonnnnnnnnnnnnnnnnoooooooooooooooooeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee')
    ...
    def runjob(self, db, job_uuid, **kw):
        print('momokkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk')
        http.request.session.db = db
        env = http.request.env(user=odoo.SUPERUSER_ID)

        def retry_postpone(job, message, seconds=None):
            print('kya karthi hai')
            job.env.clear()
            seconds = 90
        ...
        try:
            try:
                ...
                if PG_RETRY <= job.retry :
                    ...
                    raise
                retry_postpone(
                    job, tools.ustr(e, errors="replace"), seconds=PG_RETRY
                )
