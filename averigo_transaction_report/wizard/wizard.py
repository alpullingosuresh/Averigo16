...
class TransactionReportLine(models.TransientModel):
    ...
    report_id = fields.Many2one('transaction.list.report')
...
class TransactionReport(models.TransientModel):
    ...
    name = fields.Char(default=lambda self: "Transaction List ")
    mm_dom_ids = fields.Many2many('stock.warehouse')
    micro_markets = fields.Many2many('stock.warehouse', 'report_id', domain=[
    ...
    report_lines = fields.One2many('transaction.list.line', 'report_id',
                                   ondelete="cascade")
