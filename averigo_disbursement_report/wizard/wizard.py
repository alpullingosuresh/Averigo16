...
class DisbursementReportLine(models.TransientModel):
    """"""
    _name = 'disbursement.report.line'

    report_id = fields.Many2one('disbursement.report')
    branch_id = fields.Many2one('res.division', 'Operator Branch', related='micro_market.division_id', store=1)
    micro_market = fields.Many2one('stock.warehouse')
    micro_market_name = fields.Char(string='Micromarket Name', related='micro_market.name', store=1)
    operator_id = fields.Many2one('res.company', 'Operator', related='micro_market.company_id', store=1)
    ...
    brand_base_factor = fields.Selection([('margin', 'Margin'), ('net_sales', 'Net Sales'),
                                          ('gross_sales', 'Gross Sales'), ('platform_fees', 'Platform Fees')])
    management_fees = fields.Float()
    management_percentage = fields.Float()
    management_base_factor = fields.Selection([('margin', 'Margin'), ('net_sales', 'Net Sales'),
                                               ('gross_sales', 'Gross Sales'), ('platform_fees', 'Platform Fees')])
    ...
    group_base_factor = fields.Selection([('margin', 'Margin'), ('net_sales', 'Net Sales'),
                                          ('gross_sales', 'Gross Sales'), ('platform_fees', 'Platform Fees')])
    additional_fees1 = fields.Float()
    additional_fees1_percentage = fields.Float()
    additional_group1_base_factor = fields.Selection([('margin', 'Margin'), ('net_sales', 'Net Sales'),
    ...
    category = fields.Selection([('location_total', 'Location Total'), ('beer_wine', 'Beer&Wine'),
                                 ('beer_wine_customer', ' Customer Own Beer&Wine')])


class DisbursementReport(models.TransientModel):
    _name = 'disbursement.report'
    ...
    mm_dom_ids = fields.Many2many('stock.warehouse', compute='compute_partner_ids')
    micro_markets = fields.Many2many('stock.warehouse', domain=[('location_type', '=', 'micro_market')])
    start_date = fields.Date()
    end_date = fields.Date()
    report_lines = fields.One2many('disbursement.report.line', 'report_id', ondelete="cascade")
    report_length = fields.Integer()
    operator_id = fields.Many2many('res.company', default=lambda self: self.env.company)
    show_operator_column = fields.Boolean()
    show_warning = fields.Boolean()
    currency_id = fields.Many2one('res.currency', default=lambda self: self.env.company.currency_id.id)
    customer_id = fields.Many2many('res.partner', domain="[('id', 'in', partner_ids)]")
    partner_ids = fields.Many2many('res.partner', compute='compute_partner_ids')
    report_type = fields.Selection([('summary', 'Summary'), ('detail', 'Detail')], default='detail')
    excel_type = fields.Selection([('vertical', 'Vertical'), ('horizontal', 'Horizontal')], default='horizontal')
